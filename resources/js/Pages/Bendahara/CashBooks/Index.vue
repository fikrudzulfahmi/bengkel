...
const filterData = () => {
    router.get(route('bendahara.cash-books.index'), { month: month.value, year: year.value }, { preserveState: true });
};
...
const submit = () => {
    form.post(route('bendahara.cash-books.store'), {
...
const deleteEntry = (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus catatan ini?')) {
        useForm({}).delete(route('bendahara.cash-books.destroy', id));
    }
};

const isSetoran = (description) => {
    return description?.startsWith('Setoran dari') || description?.startsWith('Setor ke Bendahara');
};
</script>

<template>
    <Head title="Buku Kas Bendahara" />
        ...
        <template #header>
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Kas khusus bendahara, terpisah dari kas bengkel mekanik</p>
                </div>
                <div class="flex items-center gap-3">
                    <a :href="route('bendahara.deposits.index')" class="inline-flex items-center gap-2 text-sm font-medium bg-amber-50 text-amber-700 border border-amber-200 px-4 py-2 rounded-xl hover:bg-amber-100 transition-colors">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card class="bg-gradient-to-br from-amber-500 to-orange-500 text-white border-transparent">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <div class="text-white/80 text-sm font-medium mb-1">Saldo Kas</div>
                                ...
                                <tr v-for="cash in cashBooks" :key="cash.id" class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="p-5 text-sm text-gray-800">
                                        <div class="flex items-center gap-2">
                                            <span v-if="isSetoran(cash.description)" class="inline-flex items-center gap-1 text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">
                                    ...
                                    <td class="p-5 text-right">
                                        <button v-if="!isSetoran(cash.description)" @click="deleteEntry(cash.id)"
                                            class="text-red-500 hover:text-red-700 font-medium text-sm transition-colors p-2 hover:bg-red-50 rounded-lg">
                                            Hapus
                                        </button>
                                        <span v-else class="text-xs text-gray-400 italic">via setoran</span>
                                    </td>
                                </tr>
                            ...
                            <form @submit.prevent="submit">
                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <div class="flex items-center gap-3 mb-5">
                                        ...
                                        <h3 class="text-lg leading-6 font-bold text-gray-900" id="modal-title">Catat Kas Bendahara</h3>
                                    </div>
                                    <div class="space-y-4">
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <textarea v-model="form.description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm" required placeholder="Contoh: Pembelian ATK, Dana operasional..."></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                                    <button type="submit" :disabled="form.processing" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-amber-500 text-base font-medium text-white hover:bg-amber-600 focus:outline-none sm:w-auto sm:text-sm disabled:opacity-50">
                                        Simpan
                                    </button>
